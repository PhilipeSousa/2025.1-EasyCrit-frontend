...
const Page: React.FC = () => {
	...
	const { isConnected, error, sendMessage, disconnect } = useWebSocket({
		...
		url: websocketUrl,
		onMessage: (message) => {
			// console.log('Mensagem recebida no componente Mestre:', message); // Removido console.log
			setMessages((prevMessages) => [...prevMessages, message]) // Adiciona a mensagem ao estado
		},
		onOpen: () => {
			// console.log('Conexão WebSocket aberta no componente Mestre.'); // Removido console.log
		...
		onClose: (_event) => {
			...
			// console.log('Conexão WebSocket fechada no componente Mestre.', _event); // Removido console.log
			setWebsocketUrl(null) // Resetar a URL para permitir nova conexão
		},
		onError: (_event) => {
			// Adicionado _ para ignorar o parâmetro event
			// console.error('Erro no WebSocket no componente Mestre:', _event); // Removido console.error
